...
Neurokolo — Telegram-бот закритого клубу з рекурентною підпискою через WayForPay.
...
- Якщо активних тарифів два або більше, користувач бачить окремі персональні
  checkout-кнопки з назвою, ціною та терміном кожного тарифу.
- Тариф має період 1, 2, 3, 6 або 12 місяців; ціна вказується за весь період.
...
- Для невдалого повторного платежу subscription переходить у `past_due`: користувач
  ...
  WayForPay-регулярки (якщо URL повернув провайдер). Адмінські alerts про такі
  відмови типово вимкнені (`FAILED_PAYMENT_ADMIN_ALERTS_ENABLED=false`), а worker
  надсилає нагадування перед завершенням grace
  period і фінальне повідомлення після фактичного revoke. Успішне продовження
  очищає dunning-стан і скасовує заплановані нагадування.
...
- синхронізація персонального Telegram command scope `/admin` одразу після
  додавання/видалення та для всіх активних адміністраторів із БД після кожного запуску;
- показ фактичного Telegram ID користувача у відповіді на відмову `/admin`, щоб
  виправляти помилково доданий ID;
...
- автоматична деактивація й відв’язування ресурсу від усіх тарифів, коли бот
  втрачає права адміністратора, зі сповіщенням усіх адміністраторів;
- створення тарифів і формування їх із зареєстрованих ресурсів;
- редагування назви/ціни/терміну тарифу, безпечне архівування та відновлення
  неосновних тарифів;
...
- «Доєднатися» без entitlement пропонує персональний checkout, а з entitlement
  показує налаштований адміністратором текст/медіа та свіжі персональні
  join-request кнопки ресурсів тарифу;
- «Матеріали» показує тільки налаштований адміністратором текст/медіа/URL-кнопки
  і не створює Telegram invite links;
- налаштований контент «Техпідтримка» повністю замінює резервне повідомлення з
  `SUPPORT_USERNAME`, тому старий контакт не надсилається другим повідомленням;
- пункт статусу «Моя підписка» прихований із нижньої reply-клавіатури; команда
  `/subscription` лишається доступною для діагностики без зміни entitlement-логіки;
- URL-кнопки підтримують Telegram-стилі `primary`/синій, `success`/зелений та
  `danger`/червоний; довільні кольори Telegram Bot API не дозволяє.
...
Регулярність checkout залежить від тарифу: `monthly`, `bimonthly`, `quarterly`,
`halfyearly` або `yearly`. API регулярних платежів підтримує `STATUS`, `SUSPEND`
і `RESUME`.
...
- Адмін може редагувати назву, ціну й термін тарифу, архівувати/відновлювати
  неосновні тарифи, змінювати `/start` і post-payment повідомлення та вмикати
  ізольоване 30-хвилинне тестове вікно WayForPay.
